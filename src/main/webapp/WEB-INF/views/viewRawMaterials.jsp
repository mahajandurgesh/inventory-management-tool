        ...
        <div class="row mt-5">
            <div class="col-12">
                <div class="text-center">
                    <h2>Raw Materials in Inventory</h2>
                </div>
            </div>
        </div>
        <div class="row">
            <div class="col-12">
              <table class="table table-hover">
                ...
                <tbody>
                <%List<RawMaterial> allRawMaterials = (List<RawMaterial>)request.getAttribute("allRawMaterials"); 
                	for(RawMaterial rawMaterial : allRawMaterials){                		 
                %>
                  <tr>           
                    <th scope="row"><%=rawMaterial.getId() %></th>
                    <td><%=rawMaterial.getName() %></td>
                    <td><%=rawMaterial.getQuantity() %><%=rawMaterial.getUnit() %></td>
                    <td><%=rawMaterial.getCostPerUnit() %></td>
                    <td><a href="updateRawQuantity?id=<%=rawMaterial.getId()%>">Update</a></td>
                    <%if(rawMaterial.getQuantity()==0){ %>
                    	<td style="color:#D2122E"><b>Unavailable</b></td>
                    <%}else{ %>
                    	<td><a href="issueRawMaterial?id=<%=rawMaterial.getId()%>">Issue</a></td>
                    <%} %>
                    <td><a href="deleteRawMaterial/<%=rawMaterial.getId()%>">Delete</a></td>
                  </tr>
                  <%} %>
                </tbody>
              </table>
